...
% add salt noise
salt = 0;
if dbName=='LFW' %|| dbName=='AR' || dbName=='CFaces'
    salt = .3; % LFW
elseif dbName=='GT' || dbName=='FERET'
    salt = 0.1;
end
salt % print
...
for jClass=1:numOfClasses
    ...
    for tii=1:numOfTest % add noise
        ...
        isDouble=isa(tmpim,'double');
        if isDouble==1 % double to int8
            tmpim = uint8(tmpim);
        end
        % add salt noise 
        tmpim= imnoise(tmpim,'salt & pepper',salt);
...
for kk=1:size(trainData_0,1)
    ...
    Ai = trainData_0(kk,:); % already is double
...
for kkk=1:size(testData,1)
    ...
    Xi = testData(kkk,:); % already is double
