...
for numOfTrain=minTrains:maxTrains
    ...
    %}
    % run until find better results
    better = 0;
    for rr=1:50 
        if better < 5 % we need to find 5 cases
            MultiplicationRandom    
            % save result
            indexJson = savejson('', trainIndies, ['=' dbName '_avg_' num2str(numOfTrain) '_r' num2str(rr) '_samples.json']);
            jsonFile = ['=' dbName '_avg_' num2str(numOfTrain) '_r' num2str(rr)];
            if errorsRatioFusion<=errorsRatioSRC && errorsRatioFusion<=errorsRatioCRC
                better = better+1;                
                jsonFile = [jsonFile '_better'];
                resultAvg(numOfTrain,1)=resultAvg(numOfTrain,1)+errorsRatioSRC;
                resultAvg(numOfTrain,2)=resultAvg(numOfTrain,2)+errorsRatioCRC;
                resultAvg(numOfTrain,3)=resultAvg(numOfTrain,3)+errorsRatioFusion;
            end
            resultJson = savejson('', [errorsRatioSRC,errorsRatioCRC,errorsRatioFusion], [jsonFile '.json']);
        end
    end
